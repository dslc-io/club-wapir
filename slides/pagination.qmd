...
::: notes
-   I've under-utilized this function!
-   We'll use a package called {rappdirs}, which deals with cross-platforms user directories
-   "Standard caching headers" = "Cache-Control", "Expires", "max-age" + "Date", "Etag", "Last-Modified"
...
```{r}
#| label: httr2-pagination-req_helpers
#| eval: true
...
-   If you know how many pages you'll get, you might want to handle things more manually.
    -   Particularly because this will open up parallel requests, which we'll see shortly.
:::

## Create a list of requests

```{r}
#| label: httr2-pagination-reqs_list
#| eval: true
...
```{r}
#| label: httr2-pagination-req_perform_sequential
...
::: notes
-   We'll see how to parse this list in the next section (like everything else).
:::
...
```{r}
#| label: httr2-pagination-req_perform_parallel
