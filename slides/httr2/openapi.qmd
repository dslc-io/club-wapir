...
-   Apps usually "think" in objects
    -   Preferred data: Objects-with-properties ([example json](openapi/demo_json.json))
...
jsonlite::write_json(
  demo_json2, 
  "slides/httr2/openapi/demo_json.json", 
  auto_unbox = TRUE
)
```

[demo_json.json](openapi/demo_json.json)

```{r}
#| label: openapi-read_demo_json
#| include: false
#| eval: true
demo_json <- jsonlite::fromJSON(
  here::here("slides", "httr2", "openapi", "demo_json.json")
)
```

::: fragment
```{r}
#| label: openapi-read_demo_json-url
url <- "https://r4ds.github.io/bookclub-wapir/slides/httr2/openapi/demo_json.json"
...
## Unnesting demo_json manually

```{r}
#| label: openapi-rectangle_demo_json-manual
#| eval: true
demo_json |> 
  tibble::enframe(name = "api_id") |>
...
## Unnesting demo_json with tibblify

::: fragment
```{r}
#| label: openapi-rectangle_demo_json-tibblify_auto
#| eval: true
...
::: fragment
```{r}
#| label: openapi-rectangle_demo_json-tibblify_auto-unnest
#| eval: true
dj_tibblified |> 
...
## Unnesting manually vs tibblify

:::: columns
::: {.column .fragment}
```{r}
#| label: openapi-rectangle_demo_json-manual-side_by_side
#| eval: true
dj_tidyr <- 
  demo_json |> 
  tibble::enframe(name = "api_id") |>
...
::: {.column .fragment}
```{r}
#| label: openapi-rectangle_demo_json-tibblify_auto-side_by_side
#| eval: true
dj_tibblify <- 
  demo_json |>
  tibblify::tibblify() |> 
...
::: fragment
```{r}
#| label: openapi-rectangle_demo_json-tibblify_auto-compare
#| eval: true
waldo::compare(
...
::: fragment
```{r}
#| label: openapi-tibblify-tspec_guess
#| eval: true
...
"Yet Another Markup Language" ➡️ "YAML Ain't Markup Language"
...
::: fragment
```{yaml}
#| label: openapi-api_desc_yaml-specification
...
::: fragment
```{yaml}
#| label: openapi-apis_schema_yaml
...
```{yaml}
#| label: openapi-api_version_schema_yaml
...
```{r}
#| eval: true
tibblify(demo_json$apis.guru, tspec_api)
```
:::
::: fragment
```{r}
#| eval: true
tibblify(demo_json, tspec_apis)
```
:::
