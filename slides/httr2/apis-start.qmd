...
## How does `unnest_auto()` choose unnest direction?
...
0.  `rlang::is_named(df$col)`?
    -   `dplyr::mutate(names_col = names(col), .before = col)`
1.  `non_empties <- df$col[lengths(df$col) > 0]`
...
4.  `identical(has_null, FALSE)`?
    ...
    -   `length(common_names) == 0`?
        -   `unnest_longer(col, indices_include = TRUE)`
5.  Leftovers: `unnest_longer(col, indices_to = "descriptive_name")`

:::

::: notes
0.  Does the overall column have names? Avoid losing the names
1.  Get the names of the *non-empty* elements of the column. We'll use this for the rest of our decisions.
2.  (no change)
3.  (no change)
4.  (no change, but see validation)
    -   (no change)
        -   Validate this! Overlapping names could be coincidental (eg, first version is always "v1", some have additional versions)
5.  Leftovers: Mix of named and unnamed elements, or no shared names.
...
::: fragment
```{r}
#| label: apis_start-apis_guru_list_all_apis_unnest_auto
#| eval: true
