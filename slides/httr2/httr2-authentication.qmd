...
-   `httr2::req_cookie_preserve()` (maintain state)
-   `httr2::req_options()` (SSH keys, many other options)
-   `httr2::req_proxy()`
:::

::: notes
Is there anything in the "not covered" list that you'd like to see?
:::

# How can I use a `User-Agent` to be polite?
...
-   Default is fine almost all the time
    -   If you write a package, include user-agent settings.
    -   Documentation might ask for it (like Jim found in api.crossref.org docs)
:::

## How do I set a `User-Agent` header?
...
#> Body: empty
#> Options:
#> • useragent: 'MyUserAgent/1.0 (more details; separate with ;)'
```
